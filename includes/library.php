...
function buatKode($tabel, $inisial){
	global $koneksidb;
	// Determine key field name as first column in table
	...
	// Read defined column length from information_schema (field->length can be wrong for result sets)
	...
	// Get max numeric suffix for this prefix
	$prefixEsc = mysqli_real_escape_string($koneksidb, $inisial);
	$startPos = strlen($inisial) + 1; // SUBSTRING is 1-indexed
	...
	// Uniqueness guard (rare race) — increment until free
